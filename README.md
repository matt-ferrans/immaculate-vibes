...
| 5 | setup wizard (Railway/GitHub/CodeRabbit/Sentry/email providers) | **done (this PR)** — `immaculate-vibes setup` builds a service-onboarding plan from 6 declarative providers and writes a `.iv/handoff.{json,md}` manifest (routing table + manual-step checklist). PLAN-ONLY by design: no outward mutation, metadata-only (never secret values). The `apply` phase is a deliberate later step |
